"""Read-only 8-step Turbo evaluation for the completed timestep-exposure branch.

The four stages intentionally remain separate: ``preflight`` validates only
the exact remote archive identities; ``generate`` samples the established
24-sample diagnostic panel; ``score`` reuses authoritative PCK and CLIP; and
``report`` combines their results with the existing original and LR-only
machine-readable results.  This module has no training or checkpoint-writing
path.
"""
...
OUTPUT = TIMESTEP_TURBO_EVALUATION_ROOT
ORIGINAL_OUTPUT = ORIGINAL_TURBO_EVALUATION_ROOT
LR5E5_OUTPUT = LR5E5_TURBO_EVALUATION_ROOT
...
def _branch_spec(output: Path, args) -> tuple[Any, tuple[str, ...], dict[str, Any]]:
    ...
    assert_turbo_diagnostic_contract(spec, original_spec, branch_name="timestep-exposure")
    destination = output / "turbo_spec.json"
    if destination.is_file() and _read_json(destination) != spec:
        raise ValueError(f"Existing timestep Turbo spec conflicts with immutable diagnostic contract: {destination}")
...
def _branch_checkpoints(args) -> list[tuple[int, Path]]:
    return exact_timestep_turbo_checkpoints(
        checkpoint_dir=args.checkpoint_dir,
        hf_repo_id=args.hf_repo_id,
        hf_recovery_dir=args.hf_recovery_dir,
        steps=TIMESTEP_TURBO_CHECKPOINT_STEPS,
    )
...
def _existing_branch_rows(output: Path) -> dict[int, dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(f"Malformed timestep Turbo score results: {path}")
    result = {row.get("checkpoint_step"): row for row in rows if isinstance(row, dict)}
    if set(result) - set(TIMESTEP_TURBO_CHECKPOINT_STEPS) or len(result) != len(rows):
        raise ValueError("Timestep Turbo scores contain a checkpoint outside the completed branch")
    return result
...
def _merge_branch_generation(output: Path, generated: dict[str, list[int]]) -> dict[str, list[int]]:
    ...
    if not isinstance(previous, dict):
        raise ValueError(f"Malformed timestep generation results: {path}")
    merged: dict[str, list[int]] = {}
    for stem, steps in previous.items():
        if not isinstance(stem, str) or not isinstance(steps, list) or any(step not in TIMESTEP_TURBO_CHECKPOINT_STEPS for step in steps):
            raise ValueError("Timestep generation results contain a non-branch checkpoint")
...
def preflight(args) -> None:
    output = assert_timestep_turbo_output_isolated(args.output_dir)
    ...
    _write(output / "checkpoint_preflight.json", {
        ...
        "stems": list(stems),
        "local_checkpoint_root": str(TIMESTEP_CHECKPOINT_ROOT),
        "hf_repo_id": TIMESTEP_HF_REPO_ID,
        "hf_namespace": f"{TIMESTEP_HF_RUN_NAME}/full/",
        "checkpoints": [
            {
                "checkpoint_step": step,
                "remote_checkpoint": f"{TIMESTEP_HF_RUN_NAME}/full/step_{step:06d}.pt",
                "remote_completion_marker": f"{TIMESTEP_HF_RUN_NAME}/full/step_{step:06d}.pt.complete.json",
...
def generate(args) -> None:
    ...
    for stem in stems:
        ...
        metadata = {
            ...
            "control_path": str(controls[stem]),
        ...
        if metadata_path.exists() and _read_json(metadata_path) != metadata:
            raise ValueError(f"Existing timestep Turbo metadata conflicts with immutable contract: {metadata_path}")
        ...
        for step, checkpoint in _missing_generation_checkpoints(directory, checkpoints):
            ...
            if state["global_step"] != step:
                raise ValueError(f"Timestep checkpoint identity mismatch for step {step}")
    ...
    _write(output / "generation_results.json", {
        "metadata": turbo_metadata(),
        "stems": list(stems),
        "checkpoints": list(TIMESTEP_TURBO_CHECKPOINT_STEPS),
...
def score(args) -> None:
    ...
    unavailable = [
        {"stem": record["stem"], "pose_metric_status": "unavailable", "pose_metric_reason": "authoritative_reference_pose_unavailable",
    ...
    for step in TIMESTEP_TURBO_CHECKPOINT_STEPS:
        ...
        rows[step] = {
            "checkpoint_step": step,
            "pose": pose,
            "pose_metric_unavailable_samples": unavailable,
    ...
    _write(output / "pck_clip_results.json", {
        "metadata": turbo_metadata(), "clip_model": args.clip_model_id, "confidence_threshold": .5,
        "checkpoints": [rows[step] for step in TIMESTEP_TURBO_CHECKPOINT_STEPS],
    })
    print(output / "pck_clip_results.json")


def _required_comparison_row(path: Path, *, step: int, label: str, spec: dict[str, Any]) -> dict[str, Any]:
    assert_turbo_diagnostic_contract(spec, _read_json(path / "turbo_spec.json"), branch_name=label)
    ...
    if payload.get("metadata") != turbo_metadata() or not isinstance(payload.get("checkpoints"), list):
        raise ValueError(f"{label} Turbo score result does not match the immutable Turbo contract")
    matches = [row for row in payload["checkpoints"] if row.get("checkpoint_step") == step]
    if len(matches) != 1:
        raise ValueError(f"{label} Turbo score result must contain exactly one reusable step-{step} row")
    return matches[0]
...
def _summary_row(row: dict[str, Any], *, label: str, learning_rate: float) -> dict[str, Any]:
    ...
    return {
        ...
        **turbo_metadata(),
        "pooled_pck_005": pose["pck_005"], "pooled_pck_010": pose["pck_010"], "pooled_pck_020": pose["pck_020"],
...
def report(args) -> None:
    ...
    if not isinstance(rows, list) or tuple(row.get("checkpoint_step") for row in rows) != TIMESTEP_TURBO_CHECKPOINT_STEPS:
        raise ValueError("Timestep Turbo summary requires scores for exactly steps 1600, 1700, 1800")
    original = _required_comparison_row(Path(args.original_output_dir), step=900, label="original", spec=spec)
    lr5e5 = _required_comparison_row(Path(args.lr5e5_output_dir), step=1500, label="LR=5e-5", spec=spec)
    grid_rows = []
    for stem in stems:
        paths = [
            output / "fixed_pose" / stem / "control.png",
            Path(args.original_output_dir) / "fixed_pose" / stem / "step_000900.png",
            Path(args.lr5e5_output_dir) / "fixed_pose" / stem / "step_001500.png",
            *(output / "fixed_pose" / stem / f"step_{step:06d}.png" for step in TIMESTEP_TURBO_CHECKPOINT_STEPS),
        ]
        if not all(path.is_file() for path in paths):
            raise FileNotFoundError(f"Incomplete timestep Turbo comparison row: {stem}")
        grid_rows.append((stem, paths))
    labels = ("control", "original 900 @ 1e-4", "LR-only 1500 @ 5e-5", "timestep 1600", "timestep 1700", "timestep 1800")
    make_contact_sheet(grid_rows[:4], output / "turbo_timestep_checkpoint_selection_grid.png",
                       thumbnail_width=180, thumbnail_height=180, column_labels=labels)
    make_contact_sheet(grid_rows, output / "turbo_timestep_full_contact_sheet.png",
                       thumbnail_width=320, thumbnail_height=320, column_labels=labels)
    comparison = [
        _summary_row(original, label="original 900 @ 1e-4", learning_rate=1e-4),
        _summary_row(lr5e5, label="LR-only 1500 @ 5e-5", learning_rate=5e-5),
        *[_summary_row(row, label=f"timestep {row['checkpoint_step']}", learning_rate=5e-5) for row in rows],
    ]
    _write(output / "evaluation_summary.json", {
        "metadata": turbo_metadata(),
        "comparison": comparison,
        "original_baseline": original,
        "lr5e5_baseline": lr5e5,
        "timestep_exposure_checkpoints": rows,
        "spec_sha256": hashlib.sha256((output / "turbo_spec.json").read_bytes()).hexdigest(),
        "qualitative_grids": {"checkpoint_selection": "turbo_timestep_checkpoint_selection_grid.png", "full_contact_sheet": "turbo_timestep_full_contact_sheet.png"},
    })
...
def parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--lr5e5-output-dir", default=str(LR5E5_OUTPUT))
    ...
    common.add_argument("--checkpoint-dir", default=str(TIMESTEP_CHECKPOINT_ROOT))
    common.add_argument("--hf-repo-id", default=TIMESTEP_HF_REPO_ID)
    common.add_argument("--hf-recovery-dir", default=str(TIMESTEP_CHECKPOINT_ROOT / "hf-recovery-turbo"))
