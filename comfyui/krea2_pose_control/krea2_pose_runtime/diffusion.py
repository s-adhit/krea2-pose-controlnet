...
from .base_model.mmdit import _mask, temb
...
def patchify_and_position(latent: torch.Tensor, txt_len: int, patch: int,
    ...
    if latent.ndim != 4:
        raise ValueError(
            "patchify_and_position requires a Krea diffusion latent shaped B×C×H×W; "
            f"Qwen VAE temporal dimensions must be removed in krea2_pose_runtime.vae, got {tuple(latent.shape)}"
        )
    if latent.shape[-2] % patch or latent.shape[-1] % patch:
        raise ValueError(
            f"patchify_and_position requires H and W divisible by patch={patch}, got {tuple(latent.shape[-2:])}"
        )
